...
class IPHub_Blocker_Plugin
{

    private $option_group = 'iphub_blocker_options';
    private $option_name  = 'iphub_options';
    ...
    public function __construct()
    {
        ...
        add_action('wp_ajax_iphub_test_api', [$this, 'test_api_connectivity']);
    ...
    public function sanitize_settings($input)
    {
        ...
        // We expect only one blocking mode to be active at a time.
...
?>
     ...
     * Although instructions suggest checkboxes, only one mode should be active.
     */
    ...
    ?>
        <div class="wrap">
            <h1>IPHub Blocker Settings</h1>
            <form method="post" action="options.php">
                <?php
                settings_fields($this->option_group);
                do_settings_sections('iphub_blocker');
                submit_button();
                ?>
            </form>
            <p>
            ...
            <p>
                <button id="iphub-test-api" class="button">Test API Connectivity</button>
            </p>
            <div id="iphub-test-result" style="margin-top:10px;"></div>
        </div>
        <script type="text/javascript">
            jQuery(document).ready(function($) {
                $('#iphub-test-api').on('click', function(e) {
                    e.preventDefault();
                    var data = {
                        action: 'iphub_test_api',
                        _ajax_nonce: '<?php echo wp_create_nonce("iphub_test_api_nonce"); ?>'
                    };
                    $.post(ajaxurl, data, function(response) {
                        $('#iphub-test-result').html(response);
                    });
                });
            });
        </script>
<?php
    ...
    {
        check_ajax_referer('iphub_test_api_nonce');
        $options = get_option($this->option_name);
    ...
    public function check_visitor_ip()
    {
        ...
        $transient_key = 'iphub_check_' . md5($visitor_ip);
